...
function mergeSort(arr) {
	...
	if(!Array.isArray(arr[0])) {
		const sorted = [mergeSort(arr.slice(0, arr.length / 2)), mergeSort(arr.slice(arr.length / 2))]
...
function sorter(arr1, arr2) {
	...
	for (let i = 0; i < arr1.length; i++) {
		let prevLength = arr2.length;
		let l = 0;
		while (arr2.length !== 0 && arr1.length !== 0) {
			if (prevLength !== arr2.length) {
				prevLength = arr2.length;
				l -= 1;
			}
			if (arr1[i] > arr2[l]) {
				newArr.push(...arr2.splice(l, 1));
			} else if (arr1[i] === arr2[l]) {
				newArr = newArr.concat([...arr1.splice(i, 1), ...arr2.splice(l, 1)]);
			} else {
